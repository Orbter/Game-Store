const getWantToPlay = () => {
  fetch('https://api.igdb.com/v4/popularity_primitives/', {
    ...
    body: 'fields game_id, value;sort value desc;limit 100;where popularity_type = 2;', // convert data to JSON format for the body
  })
    .then((response) => {
      if (!response.ok) {
    ...
    .then((data) => {
      console.log('Success want to play:', data); // handle the successful response data
    })
    .catch((error) => {
      console.error('Error:', error); // handle any errors that occurred
    });
};
export { getWantToPlay };
